...
def test(args):
    ...
    model_restoration = EMDehamer().cuda() if args.cuda else EMDehamer()
    ...
    with torch.no_grad():
        for index, (x, y, name) in enumerate(tqdm(testLoader, desc='Test !!! ', file=sys.stdout), 0):
            ...
            if args.only_last:
                restored_ = model_restoration(input_, name[0], only_last=args.only_last)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_data', type=str, default='/home/zaq/data/T_Cloud/test')
    parser.add_argument('--result_dir', type=str, default='/home/zaq/Dehazing/EMPFDehamer/result/TCloud-np-best/')
    parser.add_argument('--resume_state', type=str, default='T_Cloud_np_best.pth')
